...
class NoticeMessage {
    ...
    buildRemoveMsg(namespace, room, msgIds) {
            return messageApi.buildData(
                ...
                "notice",
                messageApi.buildRoomExt(room), { type: this.noticeType.removeMsg, data: msgIds }
            );
        }
        /**
         * 删除字幕推送信息
    ...
    buildRemovePushInfo(namespace, room, position, ids) {
            return messageApi.buildData(
                ...
                messageApi.msgType.sendMsg,
                "notice",
                messageApi.buildRoomExt(room), { type: this.noticeType.pushInfo, data: { position: position, ids: ids, delete: true } }
            );
        }
        /**
         * 新增或修改字幕,需要查询数据
    ...
    buildSubmitPushInfo(namespace, room, data) {
            return messageApi.buildData(
                ...
                messageApi.msgType.sendMsg,
                "notice",
                messageApi.buildRoomExt(room), { type: this.noticeType.pushInfo, data: data }
            );
        }
        /**
         * 通知文档数据
    ...
    buildNoticeArticle(namespace, room, data) {
            return messageApi.buildData(
                ...
                messageApi.msgType.sendMsg,
                "notice",
                messageApi.buildRoomExt(room), { type: this.noticeType.articleInfo, data: data }
            );
        }
        /***
         * 离开房间
    ...
    buildLeaveRoom(namespace, room, flag) {
        return messageApi.buildData(
            ...
            messageApi.msgType.sendMsg,
            "notice",
            messageApi.buildRoomExt(room), { type: this.noticeType.leaveRoom, flag: flag }
        );
    }
    ...
    buildLeaveRoomByUserId(namespace, uuid, flag) {
            return messageApi.buildData(
                ...
                messageApi.msgType.sendMsg,
                "notice",
                messageApi.buildUserExt(null, uuid), { type: this.noticeType.leaveRoom, flag: flag }
            );
        }
        /**
         * 重复登录退出房间
    ...
    buildLeaveRoomByOtherLogin(namespace, socketId, flag) {
            let userExt = messageApi.buildUserExt(socketId, null);
            return messageApi.buildData(
                namespace,
                messageApi.msgType.sendMsg,
                "notice",
                userExt, {
                    type: this.noticeType.leaveRoom,
                    flag: flag
                }
            );
        }
    ...
    buildSendOnlineNum(namespace, room, userInfo, online) {
        return messageApi.buildData(
            ...
            messageApi.msgType.sendMsg,
            "notice",
            messageApi.buildRoomExt(room), { type: this.noticeType.onlineNum, data: { onlineUserInfo: userInfo, online: online } }
        );
    }
    ...
    buildWhChatPushInfo(namespace, to, info) {
            ...
            return messageApi.buildData(
                ...
                messageApi.msgType.sendMsg,
                "notice",
                ext, { type: this.noticeType.pushInfo, data: { position: constant.pushInfoPosition.whBox, infos: info } }
            );
    ...
    buildChatPushInfo(namespace, to, info) {
        ...
        let pushData = messageApi.buildData(
            ...
            messageApi.msgType.sendMsg,
            "notice",
            ext, { type: this.noticeType.pushInfo, data: { position: constant.pushInfoPosition.talkBox, infos: info } }
        );
        return pushData;
    }
    ...
    buildVideoPushInfo(namespace, to, info) {
        ...
        return messageApi.buildData(
            ...
            messageApi.msgType.sendMsg,
            "notice",
            ext, { type: this.noticeType.pushInfo, data: { position: constant.pushInfoPosition.videoBox, infos: info } }
        );
    }
    ...
    buildServerTimePushInfo(namespace, socketId, uuid) {
        return messageApi.buildData(
            ...
            messageApi.msgType.sendMsg,
            "notice",
            messageApi.buildUserExt(socketId, uuid), { type: this.noticeType.serverTime, data: new Date().getTime() }
        );
    }
